class queries:
  ...
  gclid_sftp_set = '''
     UPDATE google_ads.gclid_linear_attribution
     SET sftp = FALSE
     WHERE sftp IS NULL;'''
  ...
  gclid_sftp_fetch = '''
    SELECT gcl_id,
       conversion_name,
       TO_CHAR(conversion_time, 'YYYY-MM-DD HH24:MI:SS') AS conversion_time,
       attributed_credit,
       conversion_value,
       conversion_currency
  ...
  gclid_all_users_insert = '''
    INSERT INTO google_ads.gclid_linear_attribution_all_users (
    with start_date as (
        SELECT MAX(conversion_time) AS date
        FROM google_ads.gclid_linear_attribution_all_users),
         ...
         mta_flat as (
             ...
             from trade_touches_enh a
                      left join first_buy b on a.id = b.order_id
    ...
    select gcl_id
         , case when type = 'New' then 'GCLID New Buyer MTA Trades' else 'GCLID Repeat Buyer MTA Trades' end as conversion_name
  ...
  gclid_all_users_sftp_set = '''
     UPDATE google_ads.gclid_linear_attribution_all_users
     SET sftp = FALSE
     WHERE sftp IS NULL;'''



  gclid_all_users_sftp_update = '''
    UPDATE google_ads.gclid_linear_attribution_all_users
    SET sftp = TRUE
    WHERE sftp IS FALSE;
    '''
